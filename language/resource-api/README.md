...
The `get` method returns a Hash of all resources currently available, keyed by their name. If the `get` method raises an exception, the provider is marked as unavailable during the current run, and all resources of this type will fail in the current transaction. The error message will be reported to the user.
